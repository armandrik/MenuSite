import { useEffect, useRef, useState } from "react";
import { UserAccDropDown } from "./UserAccDropDown";

export const UserAccButton = () => {
  const [displayUserDropDown, setDisplayUSerDropDown] = useState(false);

  const isShopOpen = useRef<HTMLDivElement | null>(null);

  const closeShop = (event: MouseEvent) => {
    if (!isShopOpen.current?.contains(event.target as Node)) {
      setDisplayUSerDropDown(false);
    }
  };

  useEffect(() => {
    document.addEventListener("mousedown", closeShop);
  }, []);

  return (
    <div className="relative">
      <button
        onClick={() => setDisplayUSerDropDown(!displayUserDropDown)}
        className="flex items-center justify-center gap-1 px-4 h-12 font-DanaMedium bg-indigo-500 text-white rounded-xl hover:bg-indigo-600 transition-all desktop:text-sm desktop:px-3 desktop:h-11"
      >
        حساب کاربری
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 24 24"
          fill="currentColor"
          className="size-5"
        >
          <path
            fillRule="evenodd"
            d="M7.5 6a4.5 4.5 0 1 1 9 0 4.5 4.5 0 0 1-9 0ZM3.751 20.105a8.25 8.25 0 0 1 16.498 0 .75.75 0 0 1-.437.695A18.683 18.683 0 0 1 12 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 0 1-.437-.695Z"
            clipRule="evenodd"
          />
        </svg>
      </button>
      {displayUserDropDown && <UserAccDropDown isShopOpen={isShopOpen} />}
    </div>
  );
};
